...
exports.updateMe = async (req, res) => {
  try {
    ...
    if (invalidFields.length > 0) {
      throw createHttpError(400, `Champs non autorisés : ${invalidFields.join(', ')}`);
    }
    ...
    if (body.name !== undefined) {
      ...
      if (trimmed.length > 80) throw createHttpError(400, 'Le nom ne peut pas dépasser 80 caractères');
    ...
    if (body.notificationTime !== undefined) {
      if (body.notificationTime === null || body.notificationTime === '') {
        ...
        if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(value)) {
          throw createHttpError(400, 'Format invalide pour notificationTime (HH:MM)');
        }
